...
public:
    ...
    int line()
    {
        ...
        { // on parcourt la grille de bas en haut (sens décroissant de y)
            x = 0;
            test = true;
            while (test)
            {
                ...
                x++;
                if (x == nx_ + 1)
                { // si on arrive en bout de ligne c'est qu'elle est remplie
                    dscore++;
                    for (int y2 = y; y2 > 0; y2--)
                    { // fait tomber les autres cases
...
public:
    ...
    bool verify(board &game)
    {
        ...
        auto dim = game.dim();
        if (center_.second + size_[1] >= dim.second)
        { // si la pièce est en bas de la grille
            return true;
        }
        for (auto &c : shape_)
        { // si la pièce est en collision
    ...
    void move_left(board &game)
    {
        ...
        if (center_.first - size_[2] > 0)
        { // si possible dans les dimensions de la grille
            center_.first--;
            if (verify(game))
    ...
    void move_right(board &game)
    {
        ...
        if (center_.first + size_[0] < game.dim().first - 1)
        { // si possible dans les dimensions de la grille
            center_.first++;
            if (verify(game))
            {                    // vérifie si le mouvement est possible
...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        if (!(gameOver))
        {
            ...
            if (counter >= 10)
            { // fait tomber la pièce naturellement une fois sur 10
                tetromino->fall();
                if (tetromino->verify(game))
                {
                    ...
                    if (tetromino->verify(game))
                    { // on perd si la nouvelle pièce est initialement en collision
                        gameOver = true;
                    }
                }
                counter = 0;
            ...
            if (Dscore >= inverseDifficulty)
            { // accélère le jeu toutes les 5 lignes supprimées (peut être changé par le paramètre difficulty)
                Dscore = 0;
                dt = dt * 0.9;
            }
